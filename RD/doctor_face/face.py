...
@router.post("/api/face/upload")
async def upload_face_image(request: Request, image: UploadFile = File(...)) -> JSONResponse:
	...
	return JSONResponse(
		content={
			...
			"relativePath": f"data/face/{username}/{target_path.name}",
			"size": len(png_bytes),
		},
